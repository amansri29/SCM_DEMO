...
class Tms_users_api extends CI_Controller
{
  ...
  public function cust_order_delivered()
	...
	public function track_and_trace_rating()
    {
	  ...
	  $order_id            = $this->input->post('order_id');
	  $driver_id           = $this->input->post('driver_id');
	  $is_gps_enabled      = $this->input->post('is_gps_enabled'); 
        $Isvalidated       = true;
        if ($driver_id == '') {
            $message     = 'driver_id is blank';
            $Isvalidated = false;
        ...
        if ($Isvalidated) {
					...
					$query = $this->db->get();
					$OrderDetailsRows = $query->result_array();
					$gps_enabled = 'No';
					if( !empty($OrderDetailsRows)){
						foreach($OrderDetailsRows as $ORD)
						{
							$gps_enabled = strtolower($ORD['gps_enabled']); 
						}
					}
					
					$TrackAndTraceRating = 0 ;
					if( strtolower($gps_enabled) == 'yes'){
						if(  $is_gps_enabled == 'Yes'){
							$TrackAndTraceRating = 5 ;
						}else{ 
							$TrackAndTraceRating = 0 ;
						}
					}else{
			        ...
			        $last_hours = 0 ;
			        if( $seconds > 0 ){  $last_hours =( $seconds / 3600 ) ;}
						...
						/*WHEN DRIVER IS NOT ON TIME*/
							...
							if($rating->order_id==$order_id)
							{
								$update=array(
									 ...
									 'track_and_trace' => $TrackAndTraceRating,
									 'accept_and_assign' => $AcceptAndAssign
								);
								$this->db->where('order_id', $order_id);
								$this->db->where('global_id', $global_id);
								$data=$this->db->update('dbo.trans_rating', $update);
