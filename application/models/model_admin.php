 ...
 public function get_today_dispatched_order()
	...
	public function old_get_in()
	{
	 ...
	 //    $date = date('Y-m-d');
		...
		// return $this->db->get()->result_array();
		$query = $this->db->query("select  *  from dbo.sales_dispatched_order
			inner join dbo.order_details on dbo.sales_dispatched_order.order_id = order_details.order_id
			where CONVERT(varchar, gate_in_date, 23) < CONVERT(varchar, getdate(), 23)
			and shipping_status != 'Dispatched'
    ...
    public function get_delay_report()
    {
    	
		$query = $this->db->query("select * from DelayReport order by [Placement Date] desc");
    ...
    public function get_planVsDispatch_report()
    {
    	
		$query = $this->db->query("select * from PlanVsDispatch order by dispatch_date desc");
    ...
    {
		    $this->db->select('*');
			$this->db->from('dbo.notification2');
