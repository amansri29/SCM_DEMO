...
$this->load->database();
	...
	public function get_inprocess_orders()
    {
			...
			$this->db->from('dbo.sales_dispatched_order as sdo');
			$this->db->join('dbo.order_details as d','sdo.order_id = d.order_id ' , 'LEFT OUTER');
			$this->db->join('dbo.transporter as t', 't.user_id = d.trans_no and sdo.company = t.company','left outer');
			$this->db->join('dbo.customer as c', 'c.user_id = d.cust_no and sdo.company =c.company','left outer');
			$this->db->where('t.global_id', $global_id );
			//$this->db->where('sdo.status', 'Released');
	...
	public function get_bid_orders()
	{
		$user_id = $this->session->userdata('user_id');
       $date=date('Y-m-d');
		$this->db->select('*,c.name as cust_name,t.name as trans_name,d.shipping_status as status,psdo.order_id as order_id,psdo.posting_date as delivery_date,c.name as cust_name');
	...
	function InsertTransRating( $orderid='',$global_id='',$rating=''){
			$insert=array(
			 'order_id' => $orderid,
			 'global_id' => $global_id,
			 'accept_and_assign' => $rating,
		...
		$rows = $query->result_array();
		 ...
		 foreach($rows as $row)
		 {
			 $update=array(
				...
				'reason' => 'Rejected by vendor<br>Reason :'.$data['reject_reason'],
		 ...
		 // trans_rating
		 $this->InsertTransRating($data['order_id'],$global_id,0);
		/* print_r($value);
		die; */
