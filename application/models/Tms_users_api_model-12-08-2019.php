...
class Tms_users_api_model extends CI_Model
{
		 ...
		 public function get_orders($user_id,$user_type,$order_type,$global_id)
    {
         ...
         if($user_type=='customer')
		 {
			 ...
			 {	
				$this->db->select('DISTINCT(sdo.order_id) as order_id,od.order_status as order_status ,od.shipping_status as shipping_status,sdo.delivery_date as delivery_date,cast (sdo.item_code AS VARCHAR(max)) as item_code , cast (sdo.description AS VARCHAR(max)) AS description ,cast (sdo.qty_to_ship AS VARCHAR(max)) as qty_to_ship , cast (sdo.route AS VARCHAR(max)) AS route,cast (sdo.quantity AS VARCHAR(max)) AS quantity,sdo.ship_to_address,sdo.ship_to_address_2,sdo.Ship_to_Post_Code,sdo.ship_to_city,d.name as driver_name,d.mobile as driver_mobile,c.mobile as cust_mobile');
			...
			{
				$this->db->select('DISTINCT(psdo.order_id) as order_id,od.order_status as order_status , od.shipping_status as shipping_status,cast (psdo.item_code AS VARCHAR(max)) as item_code , cast (psdo.description AS VARCHAR(max)) AS description ,cast (psdo.qty_to_ship AS VARCHAR(max)) as qty_to_ship , cast (psdo.route AS VARCHAR(max)) AS route,psdo.posting_date as delivery_date, cast (psdo.quantity AS VARCHAR(max)) AS quantity,psdo.ship_to_address,psdo.ship_to_address_2,psdo.Ship_to_Post_Code,psdo.ship_to_city,d.name as driver_name,d.mobile as driver_mobile,c.mobile as cust_mobile,od.global_id,tr.customer');
				$this->db->from('dbo.posted_sales_dispatch_order as psdo');
				$this->db->join('dbo.order_details as od','psdo.order_id = od.order_id ' , 'LEFT OUTER'); 
		 ...
		 else if($user_type=='transporter')
		 {
			 if($order_type=='inprocess')
			 {	
				$this->db->select('DISTINCT(sdo.order_id)as order_id,od.order_status as order_status ,od.shipping_status as shipping_status,sdo.delivery_date as delivery_date,cast (sdo.item_code AS VARCHAR(max)) as item_code , cast (sdo.description AS VARCHAR(max)) AS description ,cast (sdo.qty_to_ship AS VARCHAR(max)) as qty_to_ship , cast (sdo.route AS VARCHAR(max)) AS route,cast (sdo.quantity AS VARCHAR(max)) AS quantity,d.id as driver_id,od.gps_enabled as gps_enabled,sdo.ship_to_address,sdo.ship_to_address_2,sdo.Ship_to_Post_Code,sdo.ship_to_city,sdo.status as sales_status,od.order_created_status as ocs_status');
				$this->db->from('dbo.sales_dispatched_order as sdo');
				$this->db->join('dbo.order_details as od','sdo.order_id = od.order_id ' , 'LEFT OUTER');
			...
			else if($order_type=='dispatched')
			{
				...
				$this->db->select('DISTINCT(sdo.order_id)as order_id,od.order_status as order_status ,od.shipping_status as shipping_status,sdo.posting_date as delivery_date,cast (sdo.item_code AS VARCHAR(max)) as item_code , cast (sdo.description AS VARCHAR(max)) AS description ,cast (sdo.qty_to_ship AS VARCHAR(max)) as qty_to_ship , cast (sdo.route AS VARCHAR(max)) AS route,cast (sdo.quantity AS VARCHAR(max)) AS quantity,d.id as driver_id,od.gps_enabled as gps_enabled,sdo.ship_to_address,sdo.ship_to_address_2,sdo.Ship_to_Post_Code,sdo.ship_to_city');
		  ...
		  else if($user_type=='driver')
		 {
			...
			{
				$this->db->select('DISTINCT(sdo.order_id) as order_id,od.order_status as order_status ,od.shipping_status as shipping_status,sdo.posting_date as delivery_date,cast (sdo.item_code AS VARCHAR(max)) as item_code , cast (sdo.description AS VARCHAR(max)) AS description ,cast (sdo.qty_to_ship AS VARCHAR(max)) as qty_to_ship , cast (sdo.route AS VARCHAR(max)) AS route,cast (sdo.quantity AS VARCHAR(max)) AS quantity,d.id as driver_id,od.gps_enabled as gps_enabled,sdo.ship_to_address,sdo.ship_to_address_2,sdo.Ship_to_Post_Code,sdo.ship_to_city ,c.mobile as cust_mobile,t.name as trans_name,od.gate_out_date');
				$this->db->from('dbo.posted_sales_dispatch_order as sdo');
				$this->db->join('dbo.order_details as od','sdo.order_id = od.order_id ' , 'LEFT OUTER');
				$this->db->join('dbo.transporter as t', 't.user_id = sdo.trans_no and sdo.company = t.company','left outer');
				$this->db->join('dbo.driver as d','d.id = od.driver_id' , 'LEFT OUTER');
			...
			{
				$this->db->select('DISTINCT(sdo.order_id) as order_id,od.order_status as order_status ,od.shipping_status as shipping_status,sdo.posting_date as delivery_date,cast (sdo.item_code AS VARCHAR(max)) as item_code , cast (sdo.description AS VARCHAR(max)) AS description ,cast (sdo.qty_to_ship AS VARCHAR(max)) as qty_to_ship , cast (sdo.route AS VARCHAR(max)) AS route,cast (sdo.quantity AS VARCHAR(max)) AS quantity,d.id as driver_id,od.gps_enabled as gps_enabled,sdo.ship_to_address,sdo.ship_to_address_2,sdo.Ship_to_Post_Code,sdo.ship_to_city,d.mobile as driver_mobile,t.name as trans_name,,c.mobile as cust_mobile');
				$this->db->from('dbo.posted_sales_dispatch_order as sdo');
				$this->db->join('dbo.order_details as od','sdo.order_id = od.order_id ' , 'LEFT OUTER');
				$this->db->join('dbo.transporter as t', 't.user_id = sdo.trans_no and sdo.company = t.company','left outer');
				$this->db->join('dbo.driver as d','d.id = od.driver_id' , 'LEFT OUTER');
