					...
					<!--<div class='trans'></div>-->
					 <?php 
									...
									<?php foreach($list as $get)
									{
									  ...
									  /*
										$SQL = "SELECT 
													 SUM(CAST(avg_rating  AS DECIMAL(10,2))) r1,
													 SUM(CAST(vehicle_condition  AS DECIMAL(10,2))) r2,
													 SUM(CAST(track_and_trace  AS DECIMAL(10,2))) r3,
													 SUM(CAST(customer  AS DECIMAL(10,2))) r4,
													 COUNT(*) AS totrow
												FROM dbo.trans_rating
												GROUP BY  global_id  
												HAVING   global_id  = '".$get['global_id']."'";
	 								 
	 									$RateCount = $this->db->query($SQL)->row();
	 								 
										$AvgRate =0;
										if( !empty($RateCount)){ 
  											$TotalRate = ($RateCount->r1+$RateCount->r2+$RateCount->r3+$RateCount->r4 );
  											$TotalRowforRate = $RateCount->totrow;
  											if( $TotalRate > 0 && $TotalRowforRate > 0 ){
  												$AvgRate = $TotalRate / $TotalRowforRate; 
  											}
										}
										*/	

										$RatingData =  $this->home->get_rating($get['global_id']);
										$AvgRating = 0 ;
										if( !empty($RatingData)){
											for( $ii = 0 ; $ii<count($RatingData);$ii++){
												if( $RatingData[$ii]['customer'] > 0 ){
													$r1 = ($RatingData[$ii]['accept_and_assign'] * 0.3);
													$r2 = ( $RatingData[$ii]['track_and_trace'] * 0.15 );
													$r3 = ( $RatingData[$ii]['vehicle_condition'] * 0.3);
													$r4 = ( $RatingData[$ii]['customer'] * .25 );
													$AvgRating += $r1+$r2+$r3+$r4;
												}else{
													$r1 = ($RatingData[$ii]['accept_and_assign'] * 0.4);
													$r2 = ( $RatingData[$ii]['track_and_trace'] * 0.20 );
													$r3 = ( $RatingData[$ii]['vehicle_condition'] * 0.40);
													$AvgRating += $r1+$r2+$r3 ;
												}
											}
											if( $AvgRating > 0 ){
 												 $AvgRating = $AvgRating / count($RatingData);
											}
											/*echo "<pre>";print_r($RatingData);echo "</pre>";*/
										}
				                     ?>
                        				<li><a href="#tab_2" class='click'data-toggle="tab"  value='<?php echo $get['global_id']?>'><i class="fa fa-user"></i> <?php echo $get['name']?><div id="transporter_<?php echo $get['global_id']?>" style='float:right'>(<?php echo  number_format((float)$AvgRating, 2, '.', '')?> )</div></a></li>
                 		<?php } ?>
                    </ul>
                </div>
						...
						RowCounter = 1;
                         $.each(obj,function(){
							...
							//	$(rating).append('<span>'+this.rating+'</span>');
								...
								document.getElementById("stars_"+$i).innerHTML = getStars(AvgRating);	
