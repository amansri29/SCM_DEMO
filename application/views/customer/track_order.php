 ...
 <head>
						...
						<section class="panel">
							...
							<form  target="_blank" role="form" id="form" class="form-horizontal" method="post" enctype="multipart/form-data"  action="">
								...
								$this->db->select('od.order_id as order_id, d.name,d.id');
									...
									$this->db->join('dbo.order_details as od','sdo.order_id = od.order_id' , 'LEFT OUTER');
									$this->db->join('dbo.driver as d','d.id = od.driver_id' , 'LEFT OUTER');
								   	$this->db->join('dbo.customer as c', 'c.user_id = od.cust_no and sdo.company =c.company','left outer');
									$this->db->where('od.order_status', 'Inprocess'); 
									$this->db->where('od.shipping_status', 'Dispatched'); 
  ...
  $("#order").change(function () {
        var end = this.value;
        $('#form').attr('action', '<?php echo base_url('index.php/customer/track_orderss');?>');	
    });
	  $("#driver").change(function () {
        var end = this.value;
        $('#form').attr('action', '<?php echo base_url('index.php/customer/track_driver');?>');	
    });
  </script>
        <!-- Firebase -->
       <script src="https://www.gstatic.com/firebasejs/5.5.5/firebase.js"></script>
		 <script>
		  // Initialize Firebase
		  var config = {
			apiKey: "AIzaSyAQtVARXK7_XJYMSMWkEe2EaCi5fsd3t30",
                ...
                var icon = { // car icon
					...
					rotation: data.val().angle //<-- Car angle
                      ...
                      var a =data.val().id;
									...
									$this->db->where('od.shipping_status', 'Dispatched'); 
									$this->db->where('od.global_id',$global_id);
									$list2 = $this->db->get()->result_array();
									//print_r($list2);
									$count=count($list1);
									$driver=array();
									foreach($list2 as $get1)
									{
										$driver=$get1['id'];
				                     ?>
					...
					$.ajax({
					type:'post',
					url:'<?php echo base_url();?>index.php/transporter/driver_list_for_map',
					data:'id='+a,
					   ...
					   return function() {
						 ...
						 // $order_id = $('#id').attr("order_id");
						  infowindow.setContent('<div>'+
							...
							'<p><b>ID: </b>'+id+'</p>'+
			                '<p><b>Name: </b>'+name+'</p>'+
			                '<p><b>Order ID </b><a target="blank" href="<?php echo base_url();?>index.php/transporter/order_view?id='+order_id+'">'+order_id+'</a></p>'+
							'<p><a class="btn btn-success btn-block" target="blank" href="<?php echo base_url();?>index.php/transporter/vehicle_track?id='+a+'">Track here</a></p>'+
							'</div>'+
							'</div>');
        ...
        <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA2_9Lpcxc2d-E1Z3oseySOeYX9aWQL2BA&callback=initMap">
        </script>
